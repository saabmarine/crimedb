...
def crimes(request):
    report_list = Report.objects.all().exclude(status='deleted')
...
def crimes_detail(request, report_id):
    ...
    invetigate_detail = Investigate.objects.all().filter(report=report_id).exclude(status="deleted")
...
#        invetigate_detail = paginator.page(paginator.num_pages)    
    return render(request, 'crimes_detail.html', {'crimes_detail':crimes_detail,'investigate_detail':invetigate_detail})
...
def homepage(request):
    ...
    for x in range(0,24):
        data.append((timeSlotChart[x],Report.objects.filter(time__startswith=timeSlot[x]).count()))

    return render(request, 'homepage.html', {'data':data})

def agents(request):
    agent_list = Agent.objects.all().filter(status='active')
...
def agents_detail(request, agent_id):
    ...
    return render(request, 'agents_detail.html', {'agents_detail':agents_detail})
...
def agents_delete(request, agent_id):
    if request.method == 'POST':
        ...
        agent.save()
        investigate = get_object_or_404(Investigate, agent=agent)
...
def suspects(request):
    suspect_list = Suspect.objects.all().filter(status='active')
...
def suspects_detail(request, suspect_id):
    ...
    return render(request, 'suspects_detail.html', {'suspects_detail':suspects_detail})
...
def suspects_delete(request,suspect_id):
    if request.method == 'POST':
        ...
        suspect.save()
        report = get_object_or_404(Report, suspect=suspect)
        report.suspect=None
        report.save()
        return HttpResponseRedirect('/crime/thanks')
    else:
...
#        locations = paginator.page(paginator.num_pages)
    ...
    for cat in categories:
        data.append((str(cat.title),round((((Report.objects.filter(category=cat).count())/count)*100),2)))
    data = sorted(data)

    for loc in locationsChart:
        data2.append((str(loc.loc_name),Report.objects.filter(location=loc).count()))
    data2 = sorted(data2)
...
def locations_detail(request, loc_id):
    ...
    for x in range(0,24):
        data.append((timeSlotChart[x],Report.objects.filter(location=locations_detail).filter(time__startswith=timeSlot[x]).count()))

    categories = Category.objects.all()
    data2=[]
    count = Report.objects.filter(location=locations_detail).count()
    for cat in categories:
        c = Report.objects.filter(category=cat,location=locations_detail).count()
        if c == 0:
            data2.append((str(cat.title),0))
        else:
            data2.append((str(cat.title),round((((Report.objects.filter(category=cat,location=locations_detail).count())/count)*100),2)))
    data2 = sorted(data2)
